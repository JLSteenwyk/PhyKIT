    ...
    def print_results(self, res_arr: list[list[str | int | float | None]]) -> None:
        if self.json_output:
            ...
            rows = [
                {
                    "status": res[0],
                    "mean_support": None if res[1] is None else round_(res[1], 4),
                    "max_support": None if res[2] is None else round_(res[2], 4),
                    "min_support": None if res[3] is None else round_(res[3], 4),
                    "stdev_support": None if res[4] is None else round_(res[4], 4),
                    "offending_taxa": (
        ...
        lines = []
        append = lines.append
        fmt_support = self._fmt_support
        for res in res_arr:
            try:
                if res[5]:
                    res[5].sort()
                    append(
                        f"{res[0]}\t{fmt_support(res[1])}\t{fmt_support(res[2])}\t{fmt_support(res[3])}\t{fmt_support(res[4])}\t{';'.join(res[5])}"
                    )
                else:
                    append(
                        f"{res[0]}\t{fmt_support(res[1])}\t{fmt_support(res[2])}\t{fmt_support(res[3])}\t{fmt_support(res[4])}"
                    )
            except IndexError:
                append(f"{res[0]}")
        if lines:
