...
- `helpers.caching` baseline import time eagerly imported `json` and `pickle`
  ...
  pass also defers `hashlib` until `_get_cache_key()` is called.
...
- `VariableSites`, `ParsimonyInformative`, and `AlignmentEntropy` baseline time
  ...
  materialization while preserving early exit for heterogeneous alignments.
...
- `SumOfPairsScore.determine_number_of_matches_and_total_pairs` baseline time
  ...
  without allocating the expected pair set on the hot path. Incomplete pair
  sets whose involved taxa have unchanged raw query/reference sequences now
  return full-match totals from the requested pair lengths before building
  arrays or entering the multiprocessing fallback.
...
- `AlignmentLengthNoGaps.get_sites_no_gaps_count` baseline time constructed a
  ...
  fall through to the existing column-reduction path. Fully identical
...
- `Dstatistic._jackknife_d_values` baseline time computed every leave-one-block
  ...
  zero-denominator blocks as zero.
...
- `LBScore.calculate_lb_score` builds the cached tree-distance table once and
  ...
  copy for every tip in the linear helper and pairwise-cache path. A later
...
- `PhylogeneticGLM._make_ultrametric` baseline time was dominated by repeated
  ...
  removed the eager `scipy.stats` import by computing two-tailed z-test p-values
  with the standard-library complementary error function. A later p-value pass
  keeps the `scipy.stats` import out of this path but uses cached
  `scipy.special.erfc` as a vectorized ufunc for coefficient z statistics,
  preserving the same two-tailed normal probabilities with far less Python loop
...
- `Dtt._compute_dtt` baseline time combined repeated root/node `tree.distance`
  ...
  Manhattan-distance formulas. DTT trapezoidal integration now uses
...
- `pgls_utils.pgls_log_likelihood` baseline time formed an explicit inverse of
  ...
  machinery inside optimizer likelihood loops.
...
- `PhyloImpute._impute_taxon` baseline time formed explicit inverses for each
  ...
  repeated two-dimensional indexing. A follow-up writer pass formats bounded
  mixed string/numeric chunks through `np.savetxt`, preserving the exact TSV text
  while reducing per-float Python formatting overhead without materializing one
  object array for the full table. Text output now batches the imputation summary
  and per-missing-value rows into one newline-joined print while preserving exact
  stdout text. A follow-up text-output pass calculates taxon and trait column
  widths in one loop instead of two generator scans, preserving byte-identical
  table formatting.
