...
| `calculate_summary_statistics_from_arr` small sequence integer detection | repeated small-list summaries for 32 integer / 128 integer / 128 equal-integer / 128 floating-point values, side-by-side previous full-list `all(...)` integer scan | 0.352552s / 0.365712s / 0.326357s / 0.089187s | 0.260778s / 0.312281s / 0.269978s / 0.083879s | 1.35x / 1.17x / 1.21x / 1.06x |
...
| `boolean_argument_parsing.str2bool` single normalization | 600k mixed lowercase / false-only / true-only parser string values, side-by-side previous double-`lower()` false path | 0.133981s / 0.234423s / 0.100520s | 0.105720s / 0.152557s / 0.100748s | 1.27x / 1.54x / 1.00x |
...
| `IdentityMatrix._parse_partitions` stripped comment check | 500k RAxML-style partition rows with whitespace-prefixed comments/blanks, comma/no-comma forms, and invalid rows | 1.235485s | 0.969558s | 1.27x |
...
| `SumOfPairsScore._process_pair_batch` cached per-taxon match masks | 4000 pair comparisons over 120 taxa x 1600 mixed-length sites / 4000 comparisons over 120 taxa x 1600 equal-length sites / 4000 comparisons over 200 taxa x 1200 mixed-length sites / 780 comparisons over 40 taxa x 50k equal-length sites / 91 comparisons over 14 taxa x 50k mixed-length sites, side-by-side previous per-pair equality masks | 0.017946s / 0.032352s / 0.022576s / 0.019127s / 0.001232s | 0.009216s / 0.008741s / 0.009556s / 0.005560s / 0.000624s | 1.95x / 3.70x / 2.36x / 3.44x / 1.97x |
...
| `EvolutionaryRatePerSite.calculate_evolutionary_rate_per_site` cached NumPy attribute proxy | 700 taxa x 6000 sites, alphabet `ACGT-?NX*`, side-by-side previous uncached lazy NumPy proxy | 0.137633s | 0.094681s | 1.45x |
...
| `CompositionalBiasPerSite.calculate_compositional_bias_per_site` cached NumPy attribute proxy | 700 taxa x 6000 sites, alphabet `ACGTN-`, side-by-side previous uncached lazy NumPy proxy | 0.154088s | 0.139214s | 1.11x |
...
| `AlignmentOutlierTaxa._constant_composition_result` precomputed rounded scalars | 500k conserved-alignment public rows, identical row dictionaries, side-by-side previous per-row `round(float(...), 4)` calls | 2.449512s | 1.528243s | 1.60x |
...
| `DiscordanceAsymmetry._fdr` in-place vector adjustment | 1M synthetic p-values, side-by-side previous temporary adjusted-expression path with identical corrected values | 0.902099s | 0.739074s | 1.22x |
...
| `TransferAnnotations._combine_child_taxa` multifurcation child union | 8192 four-child multifurcating groups plus root aggregation, side-by-side previous temporary mutable set update path | 0.080073s | 0.058076s | 1.38x |
...
| `TerminalBranchStats._get_terminal_branch_lengths_array_direct` direct list accumulation | balanced 4096 / 32768 / 131072-tip trees, side-by-side previous `np.fromiter` generator helper, randomized timing order | 0.001786s / 0.010875s / 0.177509s | 0.001367s / 0.010272s / 0.123897s | 1.31x / 1.06x / 1.43x |
...
| `NeighborNet._compute_distance_matrix_from_equal_length_sequences` clean direct validity-mask bypass | 120 x 3000 / 260 x 6000 clean ASCII DNA sites, p-distance matrix, side-by-side previous `np.isin` validity-mask guard | 0.059816s / 0.664739s | 0.042501s / 0.532415s | 1.41x / 1.25x |
...
| `parsimony_utils.retention_index` 15-state ASCII count threshold | 1000x500 / 6000x1200 / 10000x256 / 12000x1200 character-by-taxon matrices with 15 observed ASCII states, side-by-side previous 16-state `bincount` cutoff | 0.005767s / 0.178509s / 0.056769s / 0.424973s | 0.002585s / 0.157957s / 0.012939s / 0.254745s | 2.23x / 1.13x / 4.39x / 1.67x |
...
| `Chronogram` cached lazy NumPy proxy | synthetic circular arc segment builds for 128 / 2048 / 8192 arcs, randomized old/new timing order, side-by-side previous uncached lazy NumPy attribute lookup with identical sampled arc coordinates | 0.003095139s / 0.074550413s / 0.399288111s | 0.002229440s / 0.023978967s / 0.212492306s | 1.39x / 3.11x / 1.88x |
...
| `Dtt._simulate_null_avg_sq_batch` multi-trait row totals | simulated trait cubes shaped 50x512x2 / 500x512x2 / 100x2048x4, side-by-side previous top-level `np.sum(values, axis=1)` reduction while preserving the single-trait and wide-trait paths | 0.000661025s / 0.008715578s / 0.005878024s | 0.000339928s / 0.003484697s / 0.004247150s | 1.94x / 2.50x / 1.38x |
...
| `PhylogeneticOrdination._print_pca_text_output` fixed-column PCA text rows | 100k taxa x 2 / 3 / 4 PC scores plus 8 trait loadings, identical captured text, side-by-side previous generic `tolist()` row conversion | 0.398632s / 0.189873s / 0.277472s | 0.113804s / 0.132429s / 0.126973s | 3.50x / 1.43x / 2.19x |
...
| `PhylogeneticOrdination._format_pca_result` fixed-column PCA JSON rows | 100k taxa x 2 / 3 / 4 PC scores plus 8 trait loadings, identical nested payload, side-by-side previous row iteration | 0.211542s / 0.309028s / 0.273110s | 0.115732s / 0.221595s / 0.069453s | 1.83x / 1.39x / 3.93x |
...
| `PhyloPath` Fisher's C direct log accumulation | 4 / 16 / 128 p-values, randomized old/new measurement order, side-by-side previous `sum(math.log(...))` generator | 0.137236s / 0.352848s / 0.444243s | 0.066495s / 0.245612s / 0.360165s | 2.06x / 1.44x / 1.23x |
...
| `PhylogeneticSignal._pagels_lambda` cached lazy NumPy attributes | 30 repeated 44-taxon SPD VCV lambda searches, SciPy already warm, side-by-side previous lazy proxy lookup path, identical lambda/log-likelihood/p-value | 0.704360s | 0.556526s | 1.27x |
...
| `RateHeterogeneity._prepare_shared_trait_regime_data` smaller mapping scan | 1M trait taxa, 10k regime taxa, and 12k tree tips with complete trait/regime overlap, side-by-side previous full trait-key set intersection | 0.182979s | 0.013448s | 13.61x |
...
| `OUwie._shared_trait_regime_taxa` key-view intersection | 200k/200k, 100k/1M, and 250k/1M trait/regime mappings, preserving smaller-regime scan path | 0.069870s / 0.027912s / 0.309045s | 0.050935s / 0.016480s / 0.116746s | 1.37x / 1.69x / 2.65x |
...
- `Phykit.__init__` previously built the full top-level help parser and
  ...
  the invalid-value error path. The helper now lowercases non-bool parser values
  once before checking accepted true and false spellings, avoiding a second
  normalization for false values while preserving lazy `argparse` error
  construction. A later startup pass also keeps the boolean parser helper behind
  a same-name wrapper in `phykit.phykit`, so plain CLI module imports avoid the
  helper until a boolean parser option is built or invoked. That wrapper now
  caches the resolved helper after its first call, preserving lazy import
  behavior while avoiding repeated import dispatch for boolean parser values.
  Parser-description dedenting now goes through a lazy
...
- `VariableSites`, `ParsimonyInformative`, and `AlignmentEntropy` baseline time
  ...
  row dictionaries. Verbose JSON row construction now uses literal row
  dictionaries instead of repeated `dict(...)` calls while preserving the same
  `rows` and `sites` payload. Plot extent maxima now reduce through the ndarray
  method for ordinary plot arrays while preserving the generic `np.max` path for
...
- `SumOfPairsScore.determine_number_of_matches_and_total_pairs` baseline time
  ...
  path now validates equal sequence lengths with first-length scans instead of
  a temporary set of every reference and query length, then totals the per-site
  matching-pair vector with its ndarray `sum()` method, avoiding an extra lazy
  NumPy reduction dispatch. A later ordered validation pass compares directly
  against the combinations iterator instead of walking nested slices of the
  record ID list, preserving the same unordered fallback while avoiding
  repeated slice allocation. Incomplete pair sets whose involved taxa have
...
- `CompositionPerTaxon` and `GCContent` baseline time was dominated by NumPy
  ...
  Composition JSON payload rows now use literal outer dictionaries instead of
  repeated `dict(...)` wrappers while preserving the same rounded nested
  composition dictionaries.
...
- `EvolutionaryRatePerSite.calculate_evolutionary_rate_per_site` baseline time
  ...
  alignments. Repeated calculations now cache resolved NumPy attributes on the
  lazy proxy, preserving deferred import while avoiding repeated import and
  attribute dispatch during matrix construction and reductions.
  ...
  column-slice construction while preserving the same token semantics. The
  public PIC helper now computes the total count and sum of squared counts in
  one pass, avoiding per-frequency division and a second values scan while
  ...
  the generic reduction for longer matrices where it remains faster. Plot extent
  maxima now use the same direct ndarray reduction for ordinary plot arrays with
  a preserved `np.max` path for very large arrays.
- `CompositionalBiasPerSite.calculate_compositional_bias_per_site` baseline
  ...
  longer matrices where it remains faster. Plot extent maxima now use the same
  direct ndarray reduction for ordinary plot arrays with a preserved `np.max`
  path for very large arrays.
...
- `AlignmentOutlierTaxa.calculate_outliers` baseline time combined a Unicode
  ...
  rows. Constant-result rows now precompute the repeated rounded gap-rate and
  occupancy scalars once per result instead of rounding the same values for
  every taxon.
  ...
  the gapped and Unicode fallback paths. A later wide-alignment pass reduces
  same-symbol row totals through the ndarray method for 1000+ site all-valid
  matrices while keeping the previous generic reduction for short matrices where
  it remains faster. Composition-distance setup now computes
...
- `StreamingFastaReader.stream_chunks` now pulls bounded batches from the
  ...
  alignment batch processor now iterates those chunks directly, avoiding unused
  `enumerate` index construction for each batch.
...
- `Alignment.calculate_rcv` baseline time built a Unicode character matrix and
  ...
  preserving early exit for heterogeneous alignments. The shared alignment
  identity helper now uses an iterator scan rather than repeated indexed list
  access, preserving no-slice behavior while speeding identical and
  ...
  use, avoiding repeated proxy dispatch during RCV setup and reductions while
  preserving import deferral. The `rcv`
  command module
...
- `DensityMap._plot_density_map` layout setup baseline time rebuilt
  ...
  branch. A follow-up rendering pass appends each branch's posterior color rows
  in bulk and passes a numeric color matrix into the branch collections, avoiding
  per-segment color-row appends and object-list coercion. Branch posterior
  aggregation now preselects the branch histories once and advances through each
  history across sorted segment midpoints, preserving the previous
  missing-history denominator while avoiding a full history scan per segment. A
  later startup pass
...
- `QuartetNetwork._compute_quartet_cfs` baseline time represented quartet and
  ...
  output rows. The neighbor-joining circular ordering now scans active
  distances directly for each row total instead of allocating a generator for
  every active node at every merge step, while preserving distance lookup
  fallback behavior and summation order.
...
- `EvoTempoMap._test_branch` eagerly converted concordant and discordant branch
  ...
  Mann-Whitney pass handles no-tie samples locally: small samples where either
  group has at most eight observations use the same exact two-sided U
  distribution used by SciPy's automatic exact path, and larger no-tie samples
  use SciPy's continuity-corrected asymptotic normal formula. Both preserve U
  and p-values while avoiding `scipy.stats` startup for those branch tests and
  global treeness comparisons. Tied samples retain the SciPy fallback.
...
- `PolytomyTest._evaluate_tree_triplets_fast` baseline time called
  ...
  behavior while avoiding per-depth generator/list overhead. The descendant
...
- `TransferAnnotations._extract_annotations` and `_transfer` baseline time
  ...
  directly, avoiding a temporary mutable set and repeated update calls. The
  source-extraction and target-transfer child combiner now uses the same direct
  union path for multifurcations, avoiding temporary set construction in the
  command's active annotation paths. A later startup pass replaces the eager
  Bio.Phylo import with a lazy
...
- `circular_layout.compute_circular_coords` now computes tip index ranges with
  ...
  rendering. A later collection traversal pass uses direct binary child pushes
  while building radial and arc segment batches, preserving preorder segment
  setup without allocating `reversed(children)` iterators. Gradient and discrete multi-segment
...
- `NeighborNet._compute_distances_from_alignment` baseline time scanned every
  ...
  now use direct blockwise sequence comparisons after a byte scan proves there
  are no skip symbols, avoiding the previous full validity-mask allocation;
  alignments with gaps or ambiguous symbols stay on the per-symbol
  match-product path.
...
- `parsimony_utils.fitch_downpass`, `fitch_uppass_acctran`,
  ...
  competitive. The `bincount` cutoff was later lowered to 15 observed ASCII
  states after 12-state matrices remained mixed but 15-state matrices showed
  consistent wins.
...
- `Chronogram` plot setup baseline time materialized terminal clades and used
  ...
  circular plotting first computes angle and radius guides. The lazy NumPy proxy
  now caches the imported module and resolved attributes, reducing repeated
  `linspace`/trigonometric/stacking proxy overhead in circular plot setup while
  preserving import deferral. The run path now
...
- `Tree.calculate_treeness` baseline time walked nonterminals, then traversed
  ...
  The internal/total traversal now also localizes hot stack operations,
  preserving traversal behavior while trimming repeated method lookups in
  shared treeness callers.
...
- `TraitRateMap._plot_rate_map` layout setup baseline time rebuilt tip order,
  ...
  A subsequent scalar-collection pass separates gray no-rate branches from
  numeric-rate branches and attaches raw rate values to the scored
  `LineCollection`s, preserving the shared inferno color scale while avoiding
  per-branch RGBA materialization for unique rates.
...
- `Dtt._compute_dtt` baseline time combined repeated root/node `tree.distance`
  ...
  JSON output now rounds `times` and `dtt` vectors with NumPy before converting
  to Python lists, preserving six-decimal payload values while avoiding
  per-element Python `round()` on NumPy scalars for long time-series output.
...
- `ParsimonyScore._fitch_parsimony` baseline time performed a full postorder
  ...
  the direct helper default. The score-only vectorized path now counts changed
  sites with `np.count_nonzero` instead of summing the boolean vector at each
  internal node, preserving the total score while reducing per-node reduction
...
- `PhyloPath._fit_gls_from_vcv` replaces the post-lambda explicit VCV inverse
  ...
  dispatch in the model-comparison loop. A later Fisher's C pass accumulates
  those logs in a direct loop, avoiding generator-frame overhead in the small
  basis-set path while preserving summation order. Model relative likelihoods
  now likewise use scalar `math.exp` for each delta value, avoiding scalar NumPy
...
- `TerminalBranchStats.calculate_terminal_branch_stats` non-verbose used the
  ...
  collection. A later pass accumulates terminal lengths into a direct Python
  list before converting once with `np.asarray`, avoiding generator overhead in
  the non-verbose array helper. The list-returning direct helper now uses the
  ...
  Verbose JSON row construction now uses literal dictionaries instead of
  repeated `dict(...)` calls while preserving the same `rows` and `tips`
  payload.
- `calculate_summary_statistics_from_arr` and
  ...
  Small-sequence integer detection now checks the sorted endpoints first and
  only scans the interior when an all-integer result is still possible,
  preserving exact integer mean/median formatting while avoiding unnecessary
  type checks for mixed numeric summaries.
  ...
  preserving lazy import behavior while avoiding repeated proxy resolution in
  repeated array and dictionary summaries. Summary output now emits the existing
  eight-line report through one newline-joined print call, reducing stdout
  overhead in commands that report shared statistics while preserving exact text.
  A later formatter pass
