...
| `discrete_models._matrix_exp_eigendecomp_context` cheap condition estimate | nonsymmetric real-eigensystem three-state ARD context setup, 30k repeated calls, side-by-side previous SVD-based `np.linalg.cond` guard, same accept/reject result | 3.008229s | 2.216544s | 1.36x |
