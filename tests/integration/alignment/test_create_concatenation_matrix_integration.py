...
class TestCreateConcatenationMatrix(object):
    ...
    def test_create_concatenation_matrix0(self, mocked_print):
        ...
        with open(f"{here.parent.parent}/expected/create_concat_matrix.fa", "r") as expected_fa, open(
            f"{here.parent.parent}/expected/create_concat_matrix.occupancy", "r"
        ) as expected_occupency, open(
            f"{here.parent.parent}/expected/create_concat_matrix.partition"
    ...
    @patch("builtins.print")
    def test_create_concatenation_matrix2(self, mocked_print):
        prefix = "output/alignment_list_head"
        expected_start_message = dedent(
            """
			--------------------
			| General features |
			...
			| Output files |
			----------------
			Partition file output: output/alignment_list_head.partition
			Concatenated fasta output: output/alignment_list_head.fa
			Occupancy report: output/alignment_list_head.occupancy
			"""
        )
        testargs = [
            "phykit",
            "create_concatenation_matrix",
            "-a",
            f"{here.parent.parent.parent}/sample_files/alignment_list_head.txt",
            "-p",
            prefix,
        ]
        with patch.object(sys, "argv", testargs):
        ...
        with open(f"{here.parent.parent}/expected/alignment_list_head.fa", "r") as expected_fa, open(
            f"{here.parent.parent}/expected/alignment_list_head.occupancy", "r"
        ) as expected_occupency, open(
            f"{here.parent.parent}/expected/alignment_list_head.partition"
        ) as expected_partition:
            expected_fa_content = expected_fa.read()
            expected_occupency_content = expected_occupency.read()
            expected_partition_content = expected_partition.read()
