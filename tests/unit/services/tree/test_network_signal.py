...
def test_permutation_p_value_ge_counts_extreme_permutations(monkeypatch):
    ...
    original_count_nonzero = network_signal_module.np.count_nonzero
    calls = []
    ...
    monkeypatch.setattr(
        network_signal_module.np,
        "count_nonzero",
        counting_count_nonzero,
    )

    p_value = network_signal_module._permutation_p_value_ge(permutations, 2.0)

    assert p_value == pytest.approx(0.5)
    assert len(calls) == 1
    np.testing.assert_array_equal(calls[0], np.array([False, False, True, True]))
    assert np.isnan(network_signal_module._permutation_p_value_ge(np.array([]), 2.0))
...
class TestPagelsLambdaNetwork:
    ...
    def test_lambda_restores_diagonal_directly(self, monkeypatch):
        ...
        monkeypatch.setattr(network_signal_module.np, "diag", fail_diag)
        monkeypatch.setattr(
            network_signal_module.np,
            "fill_diagonal",
            fail_fill_diagonal,
