...
| `yylabel_fontsize` | `float \| None` | `None` (auto) | Font size for y-axis data labels (taxa names); `0` = hidden |
| `xylabel_fontsize` | `float \| None` | `None` (auto) | Font size for x-axis data labels (gene names); `0` = hidden |
...
**Validation:** `fig_width`, `fig_height`, and `dpi` must be positive when provided. Fontsize fields must be non-negative: `0` is accepted as a special value meaning "hide these labels" (user can pass `--ylabel-fontsize 0` or `--xlabel-fontsize 0` to explicitly hide labels). `legend_position` is validated against matplotlib's accepted `loc` values plus the special `"none"` string. Invalid values produce a clear error message and exit.

**Output format:** The output format is determined by the file extension of `--plot-output`. Matplotlib's `savefig` handles this automatically — `.png`, `.pdf`, `.svg`, and `.jpg` are all supported. The default extension remains `.png`. The `dpi` setting applies to raster formats (PNG, JPG) and is passed through but ignored by vector formats (PDF, SVG).
...
When `n_rows` or `n_cols` is `None`, those dimensions are not used in scaling — the corresponding fields get static fallback defaults (e.g., `fig_height=8.0`, `fig_width=14.0`, `ylabel_fontsize=7.0`).
...
- **ylabel_fontsize (y-axis):** `0.0` if `n_rows > 800` (meaning labels are hidden); otherwise `max(3.0, min(7.0, 7.0 - (n_rows - 50) * 0.008))` — scales from 7pt (<=50 rows) down to 3pt (~550+ rows). A `ylabel_fontsize` of `0.0` signals the plot function to hide y-axis tick labels entirely.
- **xylabel_fontsize (x-axis):** `0.0` if `n_cols > 60` (labels hidden); otherwise `max(3.0, min(7.0, 7.0 - (n_cols - 20) * 0.1))` — scales from 7pt (<=20 cols) down to 3pt (~60 cols)
...
- Sets y-axis tick label font sizes using `ylabel_fontsize` (hides labels if `0.0`)
- Sets x-axis tick label font sizes using `xylabel_fontsize` (hides labels if `0.0`)
...
--ylabel-fontsize <float>  Font size for y-axis data labels (taxa names)
...
Each plot function defines its own `default_colors` list. When the user provides `--colors`, they are merged with the defaults positionally (left to right):

| Plot needs | User provides | Result |
|------------|--------------|--------|
| 3 colors: `[A, B, C]` | 3 colors: `[X, Y, Z]` | `[X, Y, Z]` — all overridden |
| 3 colors: `[A, B, C]` | 2 colors: `[X, Y]` | `[X, Y, C]` — first two overridden, third kept as default |
| 3 colors: `[A, B, C]` | 1 color: `[X]` | `[X, B, C]` — only first overridden |
| 3 colors: `[A, B, C]` | 4 colors: `[X, Y, Z, W]` | `[X, Y, Z]` — extra ignored |
...
# Only change the "represented" color (third), keep defaults for first two
--colors ",,#e41a1c"
```

Empty entries in the comma-separated list preserve the default at that position. This lets users change a specific color without needing to know all the defaults.
...
3. In `_plot_concatenation_occupancy`, call `self.plot_config.resolve(n_rows=len(taxa), n_cols=len(alignment_paths))` then:
   ...
   - Replace hardcoded `fontsize=7` with `config.ylabel_fontsize` / `config.xylabel_fontsize`
   - Use `config.apply_to_figure()` for title, legend, and tick label font sizes
   - Use `config.dpi` in `fig.savefig()`
   - Y-axis labels auto-hidden when `ylabel_fontsize == 0.0` (handled by `apply_to_figure`)
