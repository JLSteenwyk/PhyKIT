...
| `ylabel_fontsize` | `float \| None` | `None` (auto) | Font size for y-axis data labels (taxa names); `0` = hidden |
| `xlabel_fontsize` | `float \| None` | `None` (auto) | Font size for x-axis data labels (gene names); `0` = hidden |
...
3. In `_plot_concatenation_occupancy`, call `self.plot_config.resolve(n_rows=len(taxa), n_cols=len(alignment_paths))` then:
   ...
   - Replace hardcoded `fontsize=7` with `config.ylabel_fontsize` / `config.xlabel_fontsize`
