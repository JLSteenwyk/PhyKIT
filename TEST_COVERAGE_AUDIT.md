...
| --- | --- | --- | --- | --- |
| Critical | DNA threading | Unit and CLI workflows cover ordinary sequences and optimized paths, but optimized tests compare related implementations and freeze an incorrect internal-gap result. | Use an independent codon-by-codon oracle for leading, internal, repeated, and terminal protein gaps across all-keep, masked, planned, short-DNA, and terminal-stop paths. Assert `A-C` + `AAACCC` is `AAA---CCC`; current output is `AAACCC---`. | Open production defect |
| Critical | Installed entry points | Registry/spec/wheel metadata parity is checked, and 121 manually listed tests invoke `python -m phykit`; installed targets are not loaded. | Derive tests from the registry, require every handler and module wrapper to be callable, load all 318 installed wheel scripts, and run `--help` for every canonical `pk_*` executable. The three alignment-recoding entry points currently target a missing wrapper. | Open production defect |
