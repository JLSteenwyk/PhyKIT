...
* Added ``episodic_rate_covariation`` (alias: ``erc_scan``) to identify rooted
  reference-tree clades with localized concordant or antagonistic projected
  gene-rate shifts.
* Added topological-depth-matched permutations, plus-one empirical p-values,
  and a maximum absolute clade statistic for family-wise error rate control.
* Added clade-level TSV output, significant-clade Newick annotations,
  structured JSON, per-edge local contributions, and leave-one-edge-out global
  correlation diagnostics.
* Added planted-signal and independent-null simulations, end-to-end CLI tests,
  and documentation of the statistic, exchangeability assumption, root-edge
  exclusion, and experimental status.
...
* Fixed ``monophyly_check`` crashing with
  ``TypeError: 'NoneType' object is not subscriptable`` on trees without
  internal support values (e.g., time trees). Monophyly is still well
  defined without support, so the status is now reported with ``NA`` (text)
  / ``null`` (JSON) in place of support statistics.
* ``phylogenetic_signal`` and ``trait_correlation`` now handle invariant
  (zero-variance) traits gracefully. ``phylogenetic_signal`` reports ``NA``
  (text) / ``null`` (JSON) with a warning instead of emitting ``nan``/``inf``,
  since phylogenetic signal is undefined for a constant trait.
  ``trait_correlation`` drops invariant trait columns (with a warning) before
  computing correlations and clustering, rather than crashing in hierarchical
  clustering with "The condensed distance matrix must contain only finite
  values".
* ``fit_discrete`` is substantially faster for multi-state traits. Transition
  probabilities are now computed from a single eigendecomposition of the rate
  matrix per likelihood evaluation (with an ``expm`` fallback when the
  decomposition is numerically unreliable), and the optimizer runs
  gradient-based L-BFGS-B from each starting value followed by a single
  Nelder-Mead refinement of the best result. Fitted models are unchanged; a
  5-state ER/SYM/ARD comparison on 110 taxa that previously did not finish
  within multi-minute timeouts now completes.
